...
def batch_norm_relu(inputs, training, data_format):
    inputs = tf.layers.batch_normalization(
        inputs, axis=1 if data_format == 'channel_first' else 3, momentum=_BATCH_NORM_MOMENTUM,
        epsilon=_BATCH_NORM_EPSILON, center=True, scale=True, training=training, fused=True)
    inputs = tf.nn.relu(inputs)
    return inputs


def normal_block(inputs, filters, strides, training, data_format):
    shortcut = inputs
...
def bottleneck_block(inputs, filters, strides, training, data_format):
    shortcut = inputs
...
def block_layer(inputs, block_fn, n_blocks, filters, strides, training, data_format):
    inputs = block_fn(inputs, filters, strides, training, data_format)
    for i in range(1, n_blocks):
        inputs = block_fn(inputs, filters, 1, training, data_format)
...
def resnet_main(model_function):
    ...
    classifier.train(
        input_fn=input_data.get_input_data(128, 1, True, input_data.parse_record_fn, 200)
    )

    eval_results = classifier.evaluate(
        input_fn=input_data.get_input_data(128, 1, False, input_data.parse_record_fn, 1)
    )
